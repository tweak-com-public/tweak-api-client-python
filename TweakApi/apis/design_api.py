...
class DesignApi(object):
    ...
    def designs_id_customer_get(self, id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.designs_id_customer_get(id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.designs_id_customer_get_with_http_info(id, **kwargs)
        else:
            (data) = self.designs_id_customer_get_with_http_info(id, **kwargs)
            return data

    def designs_id_customer_get_with_http_info(self, id, **kwargs):
        """
        Fetches belongsTo relation customer.
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.designs_id_customer_get_with_http_info(id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method designs_id_customer_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `designs_id_customer_get`")


        collection_formats = {}

        resource_path = '/Designs/{id}/customer'.replace('{format}', 'json')
        path_params = {}
        if 'id' in params:
            path_params['id'] = params['id']
